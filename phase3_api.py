...
# ---------------------------------------------------------------------------
# GET /tickers — proxy SEC company ticker file, cache in memory
# ---------------------------------------------------------------------------
...
def get_tickers() -> dict:
    """
    Proxy https://www.sec.gov/files/company_tickers_exchange.json and return
    a name → ticker mapping. SEC does not send CORS headers, so the frontend
    cannot fetch it directly. Cached in memory for one hour.
    """
    ...
    for row in data.get("data", []):
        _, name, ticker, _ = row
        if name and ticker and name.upper() not in tickers:
            tickers[name.upper()] = ticker
